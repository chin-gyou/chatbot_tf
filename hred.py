...
class hred(base_enc_dec):
    ...
    def word_level_rnn(self, prev_h, input_w, mask):
    ...
    def generate_encode(self,input_labels):
        if self.bi==0:
            ...
            return self.word_level_rnn(prev_h[0], input_labels[0], rolled_mask)
        else:
            rolled_mask = self.gen_mask(input_labels[2], EOU)
            h = self.word_level_rnn(prev_h[0], input_labels[0], rolled_mask)
    ...
    def run_word(self, prev_h, input_labels):
        r_mask = self.gen_mask(input_labels[1], EOU)
        return self.word_level_rnn(prev_h, input_labels[0], r_mask)
    ...
    def scan_step(self):
        ...
        if bi==0:
            ...
            _, h_s, h_d = tf.scan(self.run, [self.labels, r_h, self.rolled_label],initializer=[init_encode, init_hier, init_decoder])
        return [h_s,h_d]
